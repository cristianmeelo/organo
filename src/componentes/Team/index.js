        ...
        <section className="time" style={{ backgroundImage: "url(/images/fundo.png)", backgroundColor: hexToRgba(color, "0.6") }}>
          ...
          <div className="colaboradores">
            {collaborators.map((collaborator, index) => {
              return <Collaborator backgroundColor={color} key={index} id={collaborator.id} name={collaborator.name} role={collaborator.role} image={collaborator.image} onDelete={onDelete} />;
            })}
          </div>
        </section>
      )
